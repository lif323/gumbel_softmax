...
batch_size=3
for i in range(10):
    x = F.softmax((torch.rand(batch_size, 30, 10)), dim=-1).reshape(batch_size, -1).cuda()
    #x = torch.rand(batch_size, 300).reshape(batch_size, -1).cuda()
    recon_x = model.decode(x)
    comparison = torch.cat([recon_x.view(batch_size, 1, 28, 28), recon_x.view(batch_size, 1, 28, 28)])
    grid_array = get_grid(comparison)
    grid_array.save('test_reconstruction_hard_false' + str(i) + '.jpeg')